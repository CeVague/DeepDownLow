...
//		algo = new DictionnaireDouverture("/dictionnaires/hashmap_final_8_", moiJoueur);
	...
	/**
	 * Estime la profondeur permetant de jouer un coup rapide (en dessous du temps donné)
	 * seulon le nombre de coups joués et le facteur de branchement.
	 */
	...
	private CoupFousFous lanceAlphaBeta(Heuristique h, int profRapide, long tempsMax, int estimation){
		System.out.println("On a " + tempsMax + "ms pour trouver un bon coup");
		long heureDebut = System.currentTimeMillis();
		
		// Calcul du coup rapide
		algo = new NegAlphaBeta(h, moiJoueur, luiJoueur, profRapide);
		...
		if(estimation<0){
			...
			while(tempActuel<=(tempsMax - tempsPasseTotal - 10) && profIdeal+coupJoues<42){
				profIdeal++;
				tempActuel = (long) (-(tempActuel * 0.75)/(0.75-1));
			}
		...
		while(tempsPasseTotal<tempsMax && profIdeal+coupJoues<42){
			...
			Runnable recherche = new Runnable() {
				public void run() {
					AlgoJeu algorithme = new NegAlphaBeta(h, moiJoueur, luiJoueur, profRecherche);
			...
			if( (-(tempsPasseAB * 0.675)/(0.675-1)) < (tempsMax-tempsPasseTotal-10) ){
...
//		if(coupJoues==4 || coupJoues==5){
//			algo = new NegAlphaBeta(HeuristiquesFousFous.hlent, moiJoueur, luiJoueur, 8);
//			System.out.println("Changement d'IA pour un NegAlphaBeta peu profond.");
//		}
//		
//		if(coupJoues==8 || coupJoues==9){
//			algo = new NegAlphaBeta(HeuristiquesFousFous.hlent, moiJoueur, luiJoueur, 10);
//			System.out.println("Changement d'IA (recherche plus profonde)");
//		}
//
//		if(coupJoues==16 || coupJoues==17){
//			algo = new NegAlphaBeta(HeuristiquesFousFous.hlent, moiJoueur, luiJoueur, 17);
//			System.out.println("Changement d'IA (recherche avec un horizon de 17)");
//		}
		
		CoupFousFous coup;
		if(coupJoues<4){
			System.out.println("On utilisera un dico");
			System.out.println("\n");
			...
			coup = lanceAlphaBeta(
					...
					, 2*tempsRestant/(coupsRestantEstime - 6)
					, -2);
		}else{
			System.out.println("On utilisera AlphaBeta à profondeur 17");
			System.out.println("\n");
			algo = new NegAlphaBeta(HeuristiquesFousFous.hlent, moiJoueur, luiJoueur, 17);
			coup = (CoupFousFous) algo.meilleurCoup(plateau);
		}
